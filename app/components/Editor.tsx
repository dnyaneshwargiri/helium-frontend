...
import { createComponent, updateComponent, getAllLocalizations, createLocalization, listComponents, getComponent, clearComponents, updateLocalizationByKey, deleteComponent } from '../lib/database';
...
export default function Editor() {
  ...
  const [savedComponentsOpen, setSavedComponentsOpen] = useState<boolean>(false);
  const examples = [
      ...
      if (matches.length > 0) {
        ...
        if (componentCode.includes('export default') || componentCode.includes('function') || componentCode.includes('const')) {
          ...
          const ensureId = async () => {
            ...
            try {
              if (!id) {
                id = (globalThis as any).crypto?.randomUUID ? crypto.randomUUID() : `${Date.now()}`;
                ...
                await updateComponent(id, { code: componentCode, name: derivedName });
              }
  ...
  async function ensureLocalizationsForCode(code: string) {
    ...
    const enLowerToKey = new Map(existing.map(e => [e.en.trim().toLowerCase(), e.key]));
    ...
    while ((m = quoteRegex.exec(code)) !== null) {
      ...
      const isCodeLine = /\b(className|href)=/.test(line) || 
                         /\bhttp\b/.test(line) || 
                         /\bexport\s+(default\s+)?function/.test(line) ||
                         /return\s+\{/.test(line);
      if (
        txt.length >= 2 && /[A-Za-z]/.test(txt) &&
        !isCodeLine &&
        !isImportLine && !banned.has(lower)
      ) {
        candidates.add(txt);
      }
    }
    ...
    for (const text of candidates) {
      ...
      if (!mappedKey && enLowerToKey.has(lower)) mappedKey = enLowerToKey.get(lower)!;
      ...
      await createLocalization({
        id: (globalThis as any).crypto?.randomUUID ? crypto.randomUUID() : `${Date.now()}${Math.random()}`,
        key: finalKey,
        en: text,
        es: '', fr: '', de: '', ja: '', zh: ''
      });
      existing.push({ id: '', key: finalKey, en: text, es: '', fr: '', de: '', ja: '', zh: '' });
    ...
    while ((km = keyRegex.exec(code)) !== null) {
      ...
      if (!existing.find(e => e.key === key)) {
        const enFromKey = key.split('.').slice(-1)[0].replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
        ...
        existing.push({ id: '', key, en: enFromKey, es: '', fr: '', de: '', ja: '', zh: '' });
      }
    }
      ...
      try {
        ...
        if (data.translations && typeof data.translations === 'object') {
          ...
          for (const item of newKeys) {
            ...
            const updates: { es?: string; fr?: string; de?: string; ja?: string; zh?: string } = {};
        ...
        showToast('Auto-translation failed. Translations can be added manually.', 'warning');
      }
    }
  }
          ...
          <div className="max-w-2xl mx-auto">
            <div className="mb-6 app-surface p-5 soft-shadow">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <h1 className="text-2xl font-semibold text-gray-900 dark:text-white mb-1">React Component Creator</h1>
                  <p className="text-sm text-gray-600 dark:text-gray-400">Describe the component and I&apos;ll generate it with a live preview.</p>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div className="relative">
                    <button
                      type="button"
                      onClick={() => setSavedComponentsOpen(!savedComponentsOpen)}
                    ...
                    >
                      ...
                      <svg className={`w-3 h-3 transition-transform duration-200 ${savedComponentsOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div className="absolute top-full right-0 mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl max-h-96 overflow-y-auto min-w-[300px] z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                          {saved.length === 0 ? (
                            <div className="px-4 py-8 text-center">
                              ...
                              <p className="text-xs text-gray-500 dark:text-gray-400 font-medium">No saved components yet</p>
                            </div>
                          ) : (
                            <>
                          {saved.map(s => (
                            <div key={s.id} className="flex items-center justify-between px-4 py-3 hover:bg-gradient-to-r hover:from-blue-50 hover:to-transparent dark:hover:from-blue-900/20 dark:hover:to-transparent border-b border-gray-100 dark:border-gray-700 last:border-b-0 group transition-all duration-150 cursor-pointer">
                              <div className="flex items-center gap-2 flex-1 min-w-0">
                                {editingId === s.id ? (
                                  ...
                                  <>
                                    <button
                                      onClick={async () => {
                                        const rec = await getComponent(s.id);
                                        if (rec) {
                          ...
                          <div className="border-t border-gray-200 dark:border-gray-700 p-2">
                            <button
                              type="button"
                              onClick={async () => {
                                if (confirm('Clear all saved components?')) {
                                  try {
                                    ...
                                    setCurrentComponentId(null);
                                    setSavedComponentsOpen(false);
                                    showToast('All components cleared successfully', 'success');
                                  } catch (error) {
                        ...
                        case 'text':
                          return (
                            <div key={`${message.id}-${i}`} className="whitespace-pre-wrap">
                              {message.role === 'user' 
                                ? part.text.replace(/\(locale:\s*(en|es|fr|de|ja|zh)\)/i, '').trim()
                                : part.text}
                            </div>
                          );
                      }
                ...
                <div className="flex justify-start">
                  <div className="max-w-3xl rounded-lg px-4 py-3 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white mr-12">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-500">Generating component...</span>
                    </div>
                  </div>
                </div>
              </div>
